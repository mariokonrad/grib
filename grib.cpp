...
int main(int, char **)
{
	for (int i = 0; i < 6; i += 3) {
		...
		grib.add_var(GRIB::VAR_HGT);
		grib.add_var(GRIB::VAR_PRES);
		grib.add_var(GRIB::VAR_PRMSL);
		grib.add_var(GRIB::VAR_TMP);
		...
		GRIB::Data grib1;
		GRIB::DataRange range(data.begin(), data.end());
		...
		if (data.size()) {
			...
			std::copy(data.begin(), data.end(), std::ostream_iterator<GRIB::Data::value_type>(ofs));
		}
